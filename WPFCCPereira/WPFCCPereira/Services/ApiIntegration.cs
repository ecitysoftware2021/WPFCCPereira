            ...
            try
            {
                if (transaction != null)
                {
                    var response = await GetData(new RequestTransaction
                    {
                        ...
                        nombre1cliente = transaction.payer.NAME.Split(' ')[0] ?? ((Noun)transaction.File).nombre,
                        nombre2cliente = transaction.payer.NAME.Split(' ')[1] ?? string.Empty,
                        apellido1cliente = transaction.payer.LAST_NAME.Split(' ')[0] ?? string.Empty,
                        apellido2cliente = transaction.payer.LAST_NAME.Split(' ')[1] ?? string.Empty,
